...
unit tested; the bulk INSERT + RLS handling reuses the exact pattern
log_trade_annotation.py already verified against real Postgres (single
set_config per session, all rows same tenant in one transaction). See
...
def _tenant_id_sql(tenant_id: str | None, tenant_email: str | None) -> str:
    if tenant_id:
        return _sql_literal(tenant_id)
    return f"(SELECT id FROM tenant WHERE email = {_sql_literal(tenant_email)})"
...
def generate_trade_annotation_insert_sql(
    rows_by_symbol: dict[str, list[dict]], venue: str, tenant_id: str | None, tenant_email: str | None
) -> str:
    tenant_sql = _tenant_id_sql(tenant_id, tenant_email)
    statements = []
    for binance_symbol, rows in rows_by_symbol.items():
        ...
        for row in rows:
            columns = "tenant_id, instrument_id, ts, action, rationale_text, margin_mode, entry_fill_price, exit_fill_price, fees_paid_usd, leverage, exit_reason_real, funding_paid_usd"
            values = ", ".join(
                [
                    tenant_sql,
                    instrument_sql,
...
def generate_sql(rows_by_symbol: dict[str, list[dict]], venue: str, tenant_id: str | None, tenant_email: str | None) -> str:
    """Wrapped in an explicit BEGIN/COMMIT: SELECT set_config(...) is
    session-scoped (is_local=false), but wrapping the whole script in one
    transaction removes any ambiguity about the SQL client's own
    autocommit behavior -- the RLS context MUST be set before, and stay
    set through, every INSERT below it."""
    ...
    rls_sql = f"\nSELECT set_config('app.tenant_id', {tenant_sql}::text, false);\n"
    insert_sql = generate_trade_annotation_insert_sql(rows_by_symbol, venue, tenant_id, tenant_email)
    return f"{header}\n{instrument_sql}\n{rls_sql}\n{insert_sql}\n\nCOMMIT;\n"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--emit-sql",
        metavar="PATH",
        help="write a .sql file (instrument provisioning + set_config + INSERTs, one transaction) instead of "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.emit_sql:
        sql = generate_sql(rows_by_symbol, args.venue, args.tenant_id, args.tenant_email)
    ...
    with Session(engine) as session, session.begin():
        if args.tenant_id:
            ...
            trow = session.execute(select(Tenant.id).where(Tenant.email == args.tenant_email)).first()
            if trow is None:
                raise SystemExit(f"no tenant found with email {args.tenant_email!r}")
            tenant_id = trow.id
        ...
        for binance_symbol, rows in rows_by_symbol.items():
            ...
            for row in rows:
                session.execute(insert(TradeAnnotation).values(tenant_id=tenant_id, instrument_id=instrument_id, **row))
                inserted += 1
