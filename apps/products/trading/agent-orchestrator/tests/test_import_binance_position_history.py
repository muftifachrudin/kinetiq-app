...
# --- _sql_literal / _tenant_id_sql ---


def test_sql_literal_none_is_null():
    assert ibph._sql_literal(None) == "NULL"


def test_sql_literal_numbers_are_unquoted():
    assert ibph._sql_literal(73645.8) == "73645.8"
    assert ibph._sql_literal(3) == "3"


def test_sql_literal_escapes_embedded_quotes():
    assert ibph._sql_literal("O'Brien") == "'O''Brien'"


def test_tenant_id_sql_uses_literal_when_tenant_id_given():
    assert ibph._tenant_id_sql("abc-123", None) == "'abc-123'"


def test_tenant_id_sql_uses_subquery_when_only_email_given():
    sql = ibph._tenant_id_sql(None, "founder@example.com")
    assert sql == "(SELECT id FROM tenant WHERE email = 'founder@example.com')"


# --- generate_instrument_provision_sql / generate_trade_annotation_insert_sql / generate_sql ---


def test_generate_instrument_provision_sql_is_idempotent_and_derives_base_asset():
    sql = ibph.generate_instrument_provision_sql(["BTCUSDT"], "binance")
    assert "ON CONFLICT (venue_id, venue_symbol) DO NOTHING" in sql
    assert "'BTC/USDT:USDT'" in sql
    assert "'BTC'" in sql
    assert "WHERE name = 'binance'" in sql


def test_generate_trade_annotation_insert_sql_orders_columns_and_values_and_nulls_unknowns():
    rows_by_symbol = {
        "BTCUSDT": [
            {
                "ts": datetime.datetime(2026, 5, 29, 18, 25, 1, tzinfo=datetime.timezone(datetime.timedelta(hours=7))),
                "action": "long",
                "rationale_text": "bulk import from Binance Position History row 1 (closing_pnl=0.05239999)",
                "margin_mode": "cross",
                "entry_fill_price": 73645.8,
                "exit_fill_price": 73698.2,
                "fees_paid_usd": 0.0737,
                "leverage": None,
    ...
    sql = ibph.generate_trade_annotation_insert_sql(rows_by_symbol, "binance", "tenant-uuid", None)
    assert "INSERT INTO trade_annotation (tenant_id, instrument_id, ts, action, rationale_text, margin_mode, entry_fill_price, exit_fill_price, fees_paid_usd, leverage, exit_reason_real, funding_paid_usd)" in sql
    assert "'tenant-uuid'" in sql
    assert "'long'" in sql
    assert "'cross'" in sql
    assert "73645.8" in sql
    assert "NULL" in sql  # leverage/exit_reason_real/funding_paid_usd


def test_generate_sql_wraps_in_transaction_with_correct_ordering():
    rows_by_symbol = {
        "BTCUSDT": [
            {
                "ts": datetime.datetime(2026, 5, 29, 18, 25, 1, tzinfo=datetime.timezone.utc),
                "action": "long",
                "rationale_text": "x",
                "margin_mode": "cross",
    ...
    sql = ibph.generate_sql(rows_by_symbol, "binance", None, "founder@example.com")
    assert sql.strip().startswith("-- Generated by import_binance_position_history.py --emit-sql")
    assert sql.strip().endswith("COMMIT;")
    begin_idx = sql.index("BEGIN;")
    instrument_idx = sql.index("INSERT INTO instrument")
    set_config_idx = sql.index("SELECT set_config")
    insert_idx = sql.index("INSERT INTO trade_annotation")
    commit_idx = sql.rindex("COMMIT;")
    assert begin_idx < instrument_idx < set_config_idx < insert_idx < commit_idx
    assert "1 trade_annotation rows across 1 instruments" in sql
    assert "(SELECT id FROM tenant WHERE email = 'founder@example.com')" in sql


# --- main (--emit-sql -- no DATABASE_URL needed) ---


def test_main_emit_sql_writes_file_without_touching_db(tmp_path, capsys):
    pos_path = write(tmp_path, "positions.csv", POSITION_HISTORY_CSV)
    trade_path = write(tmp_path, "trades.csv", TRADE_HISTORY_CSV)
    out_path = tmp_path / "import.sql"
    exit_code = ibph.main(
        [
            "--emit-sql", str(out_path), "--tenant-email", "founder@example.com",
            "--position-history", pos_path, "--trade-history", trade_path, "--tz-offset-hours", "7",
        ]
    )
    assert exit_code == 0
    ...
    assert "INSERT INTO trade_annotation" in content
    assert content.count("INSERT INTO trade_annotation") == 2  # BTCUSDT + ETHUSDT closed positions
