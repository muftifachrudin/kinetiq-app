...
def test_generate_trade_annotation_insert_sql_orders_columns_and_values_and_nulls_unknowns():
    ...
    sql = ibph.generate_trade_annotation_insert_sql(rows_by_symbol, "binance")
    assert "INSERT INTO trade_annotation (instrument_id, ts, action, rationale_text, margin_mode, entry_fill_price, exit_fill_price, fees_paid_usd, leverage, exit_reason_real, funding_paid_usd)" in sql
    assert "'long'" in sql
    assert "'cross'" in sql
...
def test_generate_sql_wraps_in_transaction_with_correct_ordering():
    ...
    sql = ibph.generate_sql(rows_by_symbol, "binance")
    ...
    assert begin_idx < instrument_idx < insert_idx < commit_idx
