...
**Catatan verifikasi fallback chain**: diverifikasi end-to-end via mocked ccxt exchange object + Postgres 16 lokal beneran — kegagalan ke-1/2/3 (di bawah threshold) TIDAK trigger fallback, kegagalan ke-4 (udah >= threshold) trigger fallback & berhasil (data yg ke-tulis ke DB cocok data fallback, bukan data ccxt yg gagal, `consecutive_failures` balik ke 0), skenario "ccxt DAN fallback dua-duanya gagal" tetap gagal bersih (gak ada row nyasar ke-tulis, `consecutive_failures` tetap naik), dan skenario Hyperliquid (gak py fallback terdaftar) gak crash sama sekali (perilaku identik sblm fallback chain ada). Satu temuan gak sengaja pas testing: skenario ohlcv-tanpa-mock-fallback beneran manggil `fapi.binance.com/fapi/v1/klines` sungguhan dari sandbox ini dan dapet `451` (diblokir, konsisten sama pola blocking sandbox yg sama kayak Binance/Bybit/Hyperliquid sblmnya) — bukti tambahan wiring-nya beneran manggil endpoint asli, bukan cuma mock. **Field-shape response Binance `premiumIndex`/`klines` & Bybit `v5/market/tickers`/`v5/market/kline` di `native_fallback.py` didasarkan pada dokumentasi API publik masing-masing (bukan dites langsung ke API asli krn sandbox ini diblokir jaringan ke exchange) — verifikasi jaringan asli masih pending, sama pola kayak Bybit di connector utama.**
