...
Sengaja **flat `main.py` + `requirements.txt`**, bukan `src/` package + `pyproject.toml`: Railpack (builder Railway) punya dukungan native yang jauh lebih reliable utk `requirements.txt` drpd pyproject.toml/setuptools polos (yg auto-detect-nya sempat gagal generate step install sama sekali). Modul top-level `main.py` otomatis importable via `python -m uvicorn main:app` tanpa perlu instalasi paket sendiri. `packages/db` (skema SQLAlchemy) di-refer via editable path install (`-e ../../../packages/db` di `requirements.txt`) — sudah diverifikasi jalan lewat Railpack (bukan cuma pyproject.toml lokal kita sendiri yg butuh perlakuan khusus, sibling-package path install juga aman).
