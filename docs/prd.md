...
**Bug #1 (Railway "Build failed" — repo kosong) — FIXED, bug #2 ditemukan setelahnya:**
...
**Gap yg masih ada:**
- **Railway build failed** — lihat rencana fix di atas, blm dieksekusi.
...
- Clerk/Midtrans/XIDR: key Clerk+Midtrans sudah di-set di Railway, tapi belum dipakai kode apa pun sampai `api-gateway`/`billing` app nyata ditulis (lihat rencana fix Railway di atas, ini yg mulai isi itu).
- **XIDR/StraitsX & domain kinetiq.app**: belum di-apply/didaftarkan — actionable items di atas.
- `apps/products/trading/*` masih skeleton README-only sepenuhnya — belum ada kode aplikasi nyata (LangGraph, dsb), di luar scope fix Railway kali ini (fokus cuma `api-gateway` dulu).
