...
**Bug baru ditemukan turn ini — Railway "Build failed" (dianalisis, BELUM difix, ini rencananya):**
- **Root cause terkonfirmasi** (bukan asumsi): dicek langsung struktur repo — `apps/platform-core/api-gateway/` (dan semua service lain di `apps/platform-core/*`/`apps/products/trading/*`) **cuma berisi `README.md`**, tidak ada `pyproject.toml`/`requirements.txt`/`Dockerfile`/`Procfile` apa pun. `infra/railway/` juga kosong (cuma README), belum ada `railway.json`/`railway.toml` di mana pun di repo. Railway build log juga eksplisit: proses **Railpack** (builder otomatis Railway) men-scan seluruh isi repo, tidak nemu app yg bisa dikenali, lalu **"railpack process exited with an error"** — persis sesuai prediksi Status Implementasi ronde sebelumnya ("Railway build failed itu wajar, blm ada kode nyata").
- **Ini bukan cuma "wajar dibiarkan"** — ini juga persis item roadmap **Fase 0** yg memang harus dikerjakan sekarang ("web app minimal ... superadmin+admin panel"), jadi solusinya bukan sekadar "diamkan sampai nanti" tapi **mulai tulis `api-gateway` skeleton nyata**, sekalian benerin Railway.
- **Rencana fix** (menunggu approval, blm dieksekusi — Plan Mode):
  1. Tulis `apps/platform-core/api-gateway/` minimal: `pyproject.toml` (FastAPI+uvicorn, gaya sama spt `packages/db/pyproject.toml`), `main.py` (FastAPI app + endpoint `/health` doang dulu — tenant auth middleware/plan-gating menyusul di ronde berikutnya, jangan over-scope dari sekadar "benerin Railway build"), `railway.toml` di folder itu (build+start command eksplisit, drpd mengandalkan auto-detect Railpack yg sudah kepersi gagal sebelumnya).
  2. **Langkah manual dari user** (saya tidak py akses Railway API/dashboard): di Railway service `kinetiq-app` → Settings → Source, set **Root Directory** ke `apps/platform-core/api-gateway` — supaya Railpack/Railway build folder itu, bukan scan seluruh monorepo root.
  3. Verifikasi lokal: jalankan `uvicorn` lokal, `curl /health`, pastikan start command yg ditulis di `railway.toml` match persis dgn yg jalan lokal, sebelum push.
  4. Fix drift kecil yg ketemu sekalian: `.env.example` masih py section "Billing (Paddle, replacing Stripe)" dgn `PADDLE_API_KEY`/`PADDLE_WEBHOOK_SECRET` — belum pernah di-update ke Midtrans/XIDR/IDRX (B.16). Ganti ke `MIDTRANS_SERVER_KEY`/`MIDTRANS_CLIENT_KEY`/`MIDTRANS_MERCHANT_ID` + placeholder XIDR/IDRX credential.
...
- `apps/platform-core/api-gateway/{pyproject.toml, main.py, railway.toml}` (baru) — skeleton minimal (health-check dulu) supaya Railway build sukses; `deps.py` (tenant context middleware + product+tier plan-gating) menyusul ronde berikutnya (Section A.6/B.2)
