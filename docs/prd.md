...
- **Spec baru siap implementasi (round #2 `fib_gann_timing.py`, `docs/fib-gann-validation-brief.md` bag. 5)**: SL struktural (di balik swing basis + buffer 0.25-0.5x ATR), TP tiered dari extension level yg confluence sama garis Gann (bukan extension terdekat sembarang), R:R gate ≥1.5 (binary filter, sinyal confluence tinggi tapi R:R jelek = skip bukan diturunkan bobotnya), triple-barrier labeling (López de Prado — upper=TP1, lower=SL, vertical=timeout N-candle) utk kalibrasi bobot confluence & `trader_profile`. **Belum diimplementasi.**
