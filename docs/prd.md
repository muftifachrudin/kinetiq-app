...
**Bug nyata ditemukan (dari log CI, bukan tebakan) — perlu fix kode, bukan blocker dari user:**
- Job `neon-preview-branch` step "Run Alembic migration against the PR branch" gagal dgn `ModuleNotFoundError: No module named 'psycopg2'`. Sebab: `DATABASE_URL` yg di-output `create-branch-action` formatnya `postgresql://...` polos (bukan `postgresql+psycopg://`), jadi SQLAlchemy default coba pakai driver `psycopg2` (belum ter-install, proyek ini pakai `psycopg` v3) bukan yg sudah di-declare di `pyproject.toml`. **Fix**: normalisasi URL scheme di `packages/db/migrations/env.py` supaya selalu pakai driver `psycopg` v3 apa pun format `DATABASE_URL` yg masuk.
- **Railway "Build failed"** yg kelihatan di screenshot: ini **wajar**, bukan bug — service Railway belum ada kode aplikasi nyata utk di-build (`apps/platform-core/*` masih skeleton README-only), jadi build gagal krn tidak ada yg bisa di-deploy. Baru akan sukses setelah ada kode FastAPI/dsb nyata.
...
**Gap yg masih ada:**
- Migrasi belum pernah sukses jalan thd Neon asli (krn bug psycopg2 di atas — begitu di-fix, harusnya langsung jalan krn koneksi sudah terbukti berhasil).
- RLS policy (B.4), append-only grant `order_audit_log`, pgvector setup (C.1) — belum di-implement, butuh migrasi lanjutan stlh koneksi Neon beneran jalan.
- Clerk/Midtrans: key sudah di-set di Railway, tapi belum dipakai kode apa pun (blm ada `api-gateway`/`billing` app nyata).
- **XIDR/StraitsX & domain kinetiq.app**: belum di-apply/didaftarkan — actionable items di atas.
- `apps/platform-core/*` & `apps/products/trading/*` masih skeleton README-only — belum ada kode aplikasi nyata (FastAPI, LangGraph, dsb).
