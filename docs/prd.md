...
- ~~Kalibrasi Gann Fan price-per-time-unit~~ — **RESOLVED & sekarang SUDAH DIIMPLEMENTASI** (`gann_base_rate()`/`gann_fan_prices()` di `fib_gann_timing.py`): `price_per_time_unit = swing_price_range / swing_duration_in_bars`, diturunkan dari swing yang sama yg jadi basis fan (bukan sistem kalibrasi terpisah — satu sumber kebenaran dgn `detect_swings()` yg udah ada). **Validasi visual dari founder MASIH BELUM DILAKUKAN** — ini blocker eksplisit dari brief sblm kalibrasi dianggap reliable buat dipakai beneran. Tool bantu sudah disiapkan: `skills/strategy/scripts/gann_fan_visual_check.py` narik data instrumen asli dari DB, cetak 2 titik anchor (basis leg + pivot) buat digambar manual Gann Fan di TradingView, + tabel harga tiap dari 9 angle yg dihitung modul ini di beberapa bar ke depan — founder tinggal bandingin visual. Lihat `skills/strategy/README.md`.
