...
**Bug nyata ditemukan & sudah diperbaiki (dari log CI, bukan tebakan) — terverifikasi thd Neon asli:**
- ~~Job `neon-preview-branch` step migrasi gagal `ModuleNotFoundError: No module named 'psycopg2'`~~ — **FIXED**: `packages/db/migrations/env.py` sekarang normalisasi `DATABASE_URL` scheme ke driver `psycopg` v3 apa pun format yg masuk (pakai `make_url(...).set(drivername=...).render_as_string(hide_password=False)` — sempat salah pakai `str(url)` polos yg nge-mask password jadi `***`, sudah dikoreksi). **Terverifikasi sukses jalan migrasi thd Neon project asli** (PR #4, run `28520872439`).
- ~~Step `schema-diff-action` gagal `Branch main not found in project`~~ — **FIXED**: `ci.yml` salah asumsi nama branch Neon = `main` (ikut nama branch git), padahal branch default/utama project Neon ini bernama **`production`** (dikonfirmasi dari Neon Console). Sudah diganti `base_branch: production`. **Full pipeline (lint → create Neon branch → migrate → schema-diff) sekarang hijau penuh thd Neon asli** — pencapaian pertama.

**Railway deploy `api-gateway` — SELESAI (4 bug beruntun, semua terverifikasi via Deploy Logs asli):** service `kinetiq-app` status **Active/Online**, Deploy Logs terkonfirmasi `INFO: Uvicorn running on http://0.0.0.0:8080` diikuti `"GET /health HTTP/1.1" 200 OK` — bukan cuma healthcheck internal lolos, request nyata direspon. Riwayat 4 bug yg ditemukan & diperbaiki berurutan (didokumentasikan lengkap krn tiap bug baru muncul setelah yg sebelumnya kefix, berguna kalau pola serupa muncul lagi di service Railway berikutnya):
- ~~Root cause: `apps/platform-core/api-gateway/` cuma berisi `README.md`, Railpack scan repo tidak nemu app, "railpack process exited with an error"~~ — **FIXED**: ditulis skeleton `api-gateway` nyata (FastAPI + `/health`, `pyproject.toml`, `railway.toml`), diverifikasi lokal (`uvicorn` + `curl /health` → 200), di-merge (PR #5). User sudah set Root Directory Railway ke `apps/platform-core/api-gateway` secara manual (dikonfirmasi via screenshot) — **Build & Deploy sekarang sukses** (progress nyata, bukan gagal lagi di step ini).
- **Bug #2 ditemukan (dari screenshot deploy log user)**: setelah Build+Deploy sukses, **"Network > Healthcheck" gagal** ("Healthcheck failure"). **Root cause terverifikasi via dokumentasi resmi Railway** (bukan tebakan): *"The Railway Config File does not follow the Root Directory path. You have to specify the absolute path for the railway.json or railway.toml file"* — artinya Railway **default cari `railway.toml` di root repo** (path absolut dari root), BUKAN otomatis di dalam folder Root Directory yg di-set user. Karena `railway.toml` saya taruh di `apps/platform-core/api-gateway/railway.toml` (bukan di root repo), **Railway tidak pernah membaca config itu** — jatuh balik ke auto-detect Railpack murni utk build DAN start command, yg kemungkinan besar salah tebak start command/port binding (bukan `uvicorn kinetiq_api_gateway.main:app --host 0.0.0.0 --port $PORT` yg saya tulis), makanya container jalan (Deploy step hijau — ada proses yg start) tapi tidak pernah merespon di `/health` (healthcheck timeout).
- ~~Bug #2: `railway.toml` salah lokasi (di dalam folder service, bukan root repo)~~ — **FIXED**: dipindah ke root repo. Terverifikasi config-nya kebaca dgn benar setelah itu (log deploy berikutnya nampilin `Path: /health`, `Retry window: 30s` — persis sesuai yg ditulis), tapi healthcheck **masih gagal** dgn error baru.
- **Bug #3 ditemukan (dari tab Deploy Logs, bukan Build Logs — baru ketauan setelah user kirim log yg benar)**: container restart loop dgn `/bin/bash: line 1: uvicorn: command not found`, walau `pip install -e .` sukses install `uvicorn` (`Successfully installed ... uvicorn-0.49.0 ...` ada di build log). Root cause: Railpack kelola Python via `mise`, console-script shim (`uvicorn` binary) kemungkinan tidak ke-load ke `PATH` yg dipakai `bash -c` saat eksekusi `startCommand` custom kita — beda dgn kalau Railpack auto-detect start command sendiri (yg biasanya tau cara aktifkan shim mise dgn benar). **Sempat dicoba fix keliru**: nyangka ini soal "Unexposed service"/port ambiguity (pola bug lain yg mirip di komunitas Railway), disaranin user tambah `PORT=8080` eksplisit — user sudah coba, TETAP gagal sama persis, artinya diagnosis itu salah; baru ketauan akar masalah sebenarnya setelah lihat Deploy Logs asli.
- ~~Fix pertama: ganti `uvicorn ...` jadi `python -m uvicorn ...`~~ — user merge manual (GitHub MCP masih butuh re-auth), tapi deploy berikutnya **masih gagal**, error baru: `/mise/installs/python/3.13.14/bin/python: No module named uvicorn` — kali ini `python` KETEMU (path eksplisit kelihatan), tapi `uvicorn` modulnya tidak ada di situ, padahal `pip install -e .` di build log eksplisit bilang sukses install `uvicorn-0.49.0`.
- **Root cause sebenarnya (bug #3 direvisi)**: `[build] buildCommand = "pip install -e ."` custom di `railway.toml` **menggantikan step install native Railpack** (yg py mekanisme `python.install` khusus, tau cara persist `site-packages` dari tahap build ke image runtime final). Command mentah override kita cuma jalan di satu stage tapi tidak ter-carry ke image deploy akhir — makanya `pip install` "sukses" di log build, tapi paket itu tidak ada lagi begitu container jalan (beda image/layer). **FIXED**: hapus `buildCommand` override sepenuhnya dari `railway.toml`, biarkan Railpack pakai deteksi Python native-nya sendiri (sudah terbukti otomatis kenali "Detected Python" begitu Root Directory benar) — cuma `startCommand` (`python -m uvicorn ...`) yg tetap di-override krn Railpack tidak bisa nebak entrypoint module kita.
- User merge manual fix "hapus buildCommand" ke `main` (GitHub MCP masih belum aktif waktu itu), CI hijau — tapi deploy berikutnya **masih gagal**, error baru lagi: `No module named uvicorn` (bukan lagi "command not found" — `python`-nya ketemu tapi modulnya tidak ada). Build log kali ini kasih clue final: **tidak ada step install SAMA SEKALI** yg jalan (cuma "Packages: python 3.13.14" langsung lompat ke "Deploy") — beda dgn versi sebelumnya yg py `buildCommand` (pip install kelihatan jalan tapi hasilnya tetap hilang). Artinya dua masalah berbeda ketimpa dua kali: (a) dgn `buildCommand` custom → install jalan tapi tidak ke-persist ke runtime, (b) tanpa `buildCommand` → Railpack **tidak auto-generate install step apa pun** utk proyek `pyproject.toml`/setuptools polos (beda dgn Poetry/uv yg didukung native) — dependency memang tidak pernah ke-install.
- **FIXED (kemungkinan final)**: restrukturisasi `apps/platform-core/api-gateway/` jadi **flat `main.py` + `requirements.txt`** (drop `src/` package layout & `pyproject.toml` sepenuhnya) — Railpack py dukungan native yg jauh lebih reliable utk `requirements.txt` polos drpd pyproject.toml/setuptools. `startCommand` di `railway.toml` disesuaikan jadi `python -m uvicorn main:app ...` (tanpa prefix `kinetiq_api_gateway.` krn modul sekarang flat, otomatis importable tanpa instalasi paket sendiri). Diverifikasi lokal dgn simulasi persis cara Railpack (`pip install -r requirements.txt` di venv bersih + `python -m uvicorn main:app` + `curl /health` → 200). **GitHub MCP sudah aktif lagi** saat fix ini ditulis — kali ini saya proses PR+merge sendiri, tidak perlu manual dari user lagi.

**Riset baru turn ini:**
- **Domain**: registrar termurah utk `.app` — **Cloudflare Registrar** (harga wholesale tanpa markup, ~$? cek langsung saat checkout) atau **Porkbun**/**NameSilo** sbg alternatif (kisaran umum $5-15/tahun tergantung registrar & promo). `.app` mewajibkan HTTPS by default (sudah otomatis terpenuhi krn Railway/Vercel-style hosting selalu TLS) — tidak ada kerja ekstra dari sisi kita.
- **XIDR (StraitsX)** — user usulkan ini sbg stablecoin resmi/regulated (dibanding IDRX yg direkomendasikan sebelumnya). Terverifikasi: XIDR memang stablecoin IDR 1:1 yg **diatur/regulated**, diterbitkan StraitsX (fintech Singapura mapan, dikenal lewat XSGD). **Kabar baik**: StraitsX Indonesia Business Account **mendukung tipe "Sole Trader"** (bukan cuma Private Company/PT) — verifikasi cukup **KTP + NPWP pribadi** (mirip jalur individu/perorangan, BUKAN perlu Akta Pendirian/SK Kemenkumham spt jalur PT), proses **1-3 hari kerja**. Ini **solusi lebih baik** dari IDRX krn lebih "resmi" secara regulasi DAN tetap bisa diakses tanpa badan hukum. **Panduan apply**: (1) pastikan sudah py NIB Perorangan via OSS + NPWP pribadi (kalau belum py, bisa daftar sekalian di oss.go.id), (2) daftar StraitsX Indonesia Business Account, pilih tipe **"Sole Trader"**, (3) upload KTP + NPWP pribadi, (4) tunggu verifikasi 1-3 hari kerja, (5) begitu approved, minta akses **Payment API** (dokumentasi teknis lengkap — format request/callback dsb — belum saya verifikasi mendalam, perlu dicek langsung di dashboard StraitsX setelah akun aktif, jangan asumsikan sama persis dgn alur IDRX). **Rekomendasi**: pakai **XIDR (StraitsX) sbg provider stablecoin utama**, IDRX dicatat sbg alternatif/backup kalau StraitsX ada kendala approval.

**Gap yg masih ada:**
- RLS policy (B.4), append-only grant `order_audit_log`, pgvector setup (C.1) — belum di-implement, tinggal dikerjakan skrg krn koneksi Neon asli sudah terbukti jalan.
- Clerk/Midtrans/XIDR: key Clerk+Midtrans sudah di-set di Railway, `api-gateway` sudah live tapi baru `/health` — tenant auth middleware, plan-gating (`deps.py`), koneksi ke `packages/db`, dan endpoint bisnis nyata belum ditulis.
- **XIDR/StraitsX & domain kinetiq.app**: belum di-apply/didaftarkan — panduan lengkap di riset sebelumnya.
