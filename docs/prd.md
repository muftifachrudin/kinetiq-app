...
- ~~Kalibrasi Gann Fan price-per-time-unit~~ — **RESOLVED, formula eksak dikonfirmasi** (`docs/fib-gann-validation-brief.md` bag. 2c): `price_per_time_unit = swing_price_range / swing_duration_in_bars`, diturunkan dari swing yang sama yg jadi basis fan (bukan sistem kalibrasi terpisah — satu sumber kebenaran dgn `detect_swings()` yg udah ada). **Wajib validasi visual** stlh implementasi (eyeball-compare sample fan vs TradingView manual, founder yg verifikasi) sblm dipakai beneran. Gann Fan sendiri **masih belum diimplementasi** — kalibrasinya udah settled, tinggal dieksekusi round berikutnya.
