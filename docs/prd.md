...
- **Kalibrasi Gann Fan price-per-time-unit** — founder pakai default viewport TradingView (gak portable ke backend headless). 2 opsi didiskusikan (fixed reference scale dari ATR/swing range vs approximate viewport logic) — condong opsi 1 (lebih portable/programmatic), tunggu konfirmasi eksplisit founder sblm implementasi Gann jalan.
