...
- ~~Bug #3: `uvicorn` command not found~~ — **FIXED**: ganti `startCommand` di `railway.toml` dari `uvicorn ...` (bare command, bergantung PATH) jadi `python -m uvicorn ...` (`python` sendiri terbukti ada di PATH, cara ini tidak bergantung shim mise utk resolve executable `uvicorn`). Diverifikasi lokal (`python -m uvicorn` + `curl /health` → 200). **Push berhasil ke branch relay, TAPI belum bisa dibuatkan PR/merge ke `main`** — GitHub MCP connector butuh re-autorisasi (sesi non-interactive, tidak bisa jalankan OAuth sendiri) — lihat catatan di bawah, ini blocker aktif skrg.
