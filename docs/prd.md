...
- Risk hard gate: `execution/risk_gate.py` (v1, 13 Juli 2026) sudah ADA — kill-switch, symbol-universe permission, dan defensive re-check R:R/entry-validity. Regime gate (FREEZE/RISK_OFF) dan kNN risk memory veto: desain + kode `GateConfig` (`volatility_regime_only`/`knn_risk_memory_only`, `gated_campaign.py`) selesai, **dan validasi walk-forward NYATA sudah dijalankan (14 Juli 2026, 4 seri produksi BTC/ETH x Binance/Bybit, ~3 tahun data) — HASIL: kedua gate GAGAL lolos kriteria adopsi.** `volatility_regime_only`: PF net turun tipis di 3/4 seri, window lolos PF tertinggi cuma 11/35 (31%, ambang 66,66%). `knn_risk_memory_only`: veto 89-93% sinyal DAN PF net lebih buruk dari baseline di semua 4 seri dgn default; **sweep grid k∈{5,10,20}×threshold∈{0.5,0.6,0.7} (14 Juli 2026, `gated_campaign_knn_sweep.json`) TETAP gagal promosi (terbaik 14/35=40%)** — tapi menemukan asimetri lintas-koin konsisten: `k10,t0.7` membaik di KEDUA seri ETH (PF net di atas baseline + window 10→14) tapi memburuk di KEDUA seri BTC (selaras temuan "edge tidak generalize lintas-koin"). **Tidak ada yang di-wire ke produksi** — angka lengkap di `docs/validation-results/gated_campaign.json` (+`_knn_sweep.json`) & `docs/kanban.md`. Daily-loss-limit/drawdown kill-switch & exposure cap: desain (`docs/daily-loss-limit-exposure-cap-brief.md`) + **migrasi skema selesai** (0011: `position.status`/`exit_price`/`realized_pnl_usd` + tabel `equity_snapshot`, diuji round-trip lokal termasuk backfill thd data existing, 14 Juli 2026) — formula hard-coded reuse `RiskMandate.max_daily_loss_usd`/`max_drawdown_pct` (ada sejak migrasi 0001, belum pernah dipakai), exposure cap v1 reuse formula margin-ratio-cap `margin-mode-brief.md` §7 (correlation-based sungguhan tetap ditunda). **Wiring ke `execution/risk_gate.py` DAN penerapan migrasi ke production Neon belum** — butuh orkestrator live (`custody/`/`graphs/` masih kosong) yang belum ada sama sekali untuk benar-benar menulis ke kolom/tabel baru ini.
...
[Layer 3 — Risk Hard Gate]  ← veto berlapis, deterministik
  1. Regime gate (FREEZE/RISK_OFF → no-trade) — desain+kode+validasi walk-forward SELESAI, **GAGAL promosi** (window lolos PF terbaik 11/35, ambang 66,66%) — belum di-wire
  2. kNN risk memory (veto/size-down berbasis kemiripan histori rugi) — desain+kode+validasi walk-forward SELESAI, **GAGAL promosi** (veto 89-93% sinyal, PF net lebih buruk dari baseline di semua seri) — belum di-wire
  3. R:R gate ≥1.5 + kill-switch + symbol-universe permission — ADA (`execution/risk_gate.py` v1, `passes_risk_reward_gate()`)
  4. Exposure caps (leverage, korelasi, daily loss, cooldown) — leverage/notional sudah di `position_sizing.py` (downstream, bukan gate); daily-loss/drawdown/margin-ratio-cap desain SELESAI (`docs/daily-loss-limit-exposure-cap-brief.md`), implementasi belum; korelasi sungguhan & cooldown masih belum ada desain
        ▼
...
## 4. Data Model (Current — `packages/db/src/kinetiq_db/models.py`, post-migration 0011, tanpa `tenant_id`)
...
| `strategy`, `portfolio_target`, `position` | State strategi & posisi (paper/live via `is_paper`); `position` sejak migrasi 0011 punya `status`/`exit_price`/`realized_pnl_usd` (`docs/daily-loss-limit-exposure-cap-brief.md`) — schema-only, belum ada yang menulis (orkestrator live belum ada) |
| `equity_snapshot` | BARU (migrasi 0011) — ledger equity periodik (`account_id`, `ts`, `equity_usd`, realized/unrealized PnL), fondasi utk daily-loss-limit/drawdown kill-switch; belum ditulis proses manapun |
| `order_audit_log` | Append-only (trigger `BEFORE UPDATE OR DELETE`, berlaku bahkan thd table owner) |
| `risk_mandate` | Hard cap per `account_id` (max_leverage, max_drawdown_pct, kill_switch_active, dst) — PK sekarang `account_id` saja (bukan lagi composite dgn tenant_id); `max_daily_loss_usd`/`max_drawdown_pct` ada sejak migrasi 0001 tapi belum dipakai kode apa pun |
...
**Tujuan**: dari sinyal jadi keputusan ter-gate. **Status: PARSIAL.** `fit_weights.py` (meta-model per-regime, refit per window) sudah ada tapi median AUC OOS 0.522 — **exit gate PF net ≥1.0 di ≥4/6 window BELUM diverifikasi lolos** (run pertama 2/10 window; kombinasi HTF+R:R band masih hipotesis in-sample). Risk hard gate: `execution/risk_gate.py` v1 (13 Juli 2026) sudah ADA — kill-switch, symbol-universe permission, defensive re-check R:R/entry-validity, pure function & DB-free. Regime gate & kNN risk memory veto: desain + kode + **validasi walk-forward nyata selesai** (`docs/regime-gate-knn-risk-memory-brief.md`, `gated_campaign.py`'s `volatility_regime_only`/`knn_risk_memory_only`, 14 Juli 2026, 4 seri produksi; kNN termasuk sweep grid penuh) — **hasil: GAGAL lolos kriteria adopsi** (kNN sweep tetap gagal tapi menemukan edge per-koin ETH), tidak di-wire ke produksi (angka lengkap `docs/validation-results/gated_campaign.json`, `_knn_sweep.json`). Exposure caps runtime: daily-loss/drawdown/margin-ratio-cap desain selesai (`docs/daily-loss-limit-exposure-cap-brief.md`), implementasi belum; korelasi sungguhan/cooldown masih belum ada desain. LLM Arbiter opsional (feature-flag OFF default, dibandingkan A/B vs meta-model murni) belum dibangun. Baseline harness (vs buy-and-hold, vs MARKOVIZ logistic lama) sebagian ada lewat `metrics.py`.
...
**Tujuan**: live dgn blast radius minimal. **Status: BELUM DIMULAI.** Kill-switch MANUAL (`RiskMandateSnapshot.kill_switch_active`, dicek `execution/risk_gate.py` v1) sudah ADA; kill-switch OTOMATIS (DD 20%→flat, daily loss limit, anomaly detector, heartbeat monitor): desain + migrasi skema selesai (`docs/daily-loss-limit-exposure-cap-brief.md`, migrasi 0011), TAPI belum di-wire ke `risk_gate.py` — butuh orkestrator live dulu utk benar-benar menulis running-PnL. `execution/custody/` (order/position adapter, key vault) masih skeleton kosong. `position_sizing.py` (F7a, PreTradeCard) sudah ada sbg building block paralel.
...
| Risk hard gate — regime gate & kNN risk memory (`docs/regime-gate-knn-risk-memory-brief.md`, `gated_campaign.py`) | ⚠️ Desain+kode+validasi walk-forward selesai (14 Juli 2026) — **GAGAL promosi**, tidak di-wire ke produksi |
| Risk hard gate — daily-loss/drawdown/margin-ratio-cap (`docs/daily-loss-limit-exposure-cap-brief.md`, migrasi 0011) | ⚠️ Desain+migrasi skema selesai (14 Juli 2026), wiring ke `risk_gate.py` + orkestrator live belum |
| Risk hard gate — correlation-based exposure cap sungguhan & cooldown | 🆕 Belum dibangun, belum ada desain |
